    ...
    while Config.Farming do
        local success, error = pcall(function()
            ...
            if Config.AutoQuest then
                local questComplete = player.PlayerFolder.CurrentQuest.Complete:FindFirstChild("Aogiri Member")
                if not questComplete or questComplete.Value >= questComplete.Max.Value then
                    GetQuest()
                    task.wait(1)
                    return
                end
