...
local fire = {
    "https://static.vecteezy.com/system/resources/thumbnails/052/546/291/small/3d-number-11-eleven-made-up-of-blue-body-with-silver-outline-3d-illustration-png.png",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ3OZWMRhZKPKDdFb6lvRh4aD0poXI1MW0IQQ&s"
}
...
local function HandleSummons()
    ...
    if Toggles.PityBossFarm.Value then
        ...
        if isUseTurn and useName then
            ...
            -- Dù summonable hay không, khi isUseTurn thì không summon build boss nữa
            return
        end
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SECTION 8: UI LAYOUT — COMPLETE REDESIGN
-- New Tab Order: Overview | Farm | Combat | Automation | Artifact | Dungeon
--                Player   | Teleport | Misc | Webhook | Config
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ── Tab Definitions ──────────────────────────────────────────────────────────
local Tabs = {
    Overview   = Window:AddTab("Overview",    "layout-dashboard"),
...
-- Aliases for compatibility with existing logic references
Tabs.Info        = Tabs.Overview
Tabs.Auto        = Tabs.Automation
Tabs.Boss        = Tabs.Farm
Tabs.Main        = Tabs.Farm
Tabs.Information = Tabs.Overview
Tabs.Priority    = Tabs.Farm

-- ── Top-Level Groupboxes (GB) ────────────────────────────────────────────────
local GB = {
    Overview = {
        Left  = {
            Session = Tabs.Overview:AddLeftGroupbox("Session",   "activity"),
            Actions = Tabs.Overview:AddLeftGroupbox("Actions",   "play"),
        },
        Right = {
            Priority = Tabs.Overview:AddRightGroupbox("Priority", "arrow-up-down"),
            Info     = Tabs.Overview:AddRightGroupbox("Info",     "info"),
        },
    },
    Artifact = {
        Left  = {
...
local TB = {
    Farm = {
        Left  = { FarmBox   = Tabs.Farm:AddLeftTabbox()  },
        Right = { SwitchBox = Tabs.Farm:AddRightTabbox() },
    },
    Combat = {
        Left  = { CombatBox = Tabs.Combat:AddLeftTabbox()  },
        Right = { WeaponBox = Tabs.Combat:AddRightTabbox() },
    },
    Automation = {
        Left  = {
            StatsBox = Tabs.Automation:AddLeftTabbox(),
            RollsBox = Tabs.Automation:AddLeftTabbox(),
        },
        Right = {
            EnchantBox = Tabs.Automation:AddRightTabbox(),
        },
    },
    Dungeon = {
        Left  = { DungeonBox = Tabs.Dungeon:AddLeftTabbox() },
        Right = {},
    },
    Teleport = {
        Left  = { WaypointBox = Tabs.Teleport:AddLeftTabbox()  },
        Right = { NPCBox      = Tabs.Teleport:AddRightTabbox() },
    },
    Misc = {
        Left  = { MiscLeftBox = Tabs.Misc:AddLeftTabbox()  },
        Right = { PuzzleBox   = Tabs.Misc:AddRightTabbox() },
    },
}

-- ── Tab Instances (TB_Tabs) ──────────────────────────────────────────────────
local TB_Tabs = {
    -- Farm: left = mob/world/summon/pity/alt  |  right = title/rune/build
    Farm = {
        Mob       = TB.Farm.Left.FarmBox:AddTab("Mob"),
        WorldBoss = TB.Farm.Left.FarmBox:AddTab("World Boss"),
        Summon    = TB.Farm.Left.FarmBox:AddTab("Summon"),
        Pity      = TB.Farm.Left.FarmBox:AddTab("Pity"),
        Alt       = TB.Farm.Left.FarmBox:AddTab("Alt Help"),
    },
    Switch = {
        Title = TB.Farm.Right.SwitchBox:AddTab("Title"),
        Rune  = TB.Farm.Right.SwitchBox:AddTab("Rune"),
        Build = TB.Farm.Right.SwitchBox:AddTab("Build"),
    },
    -- Combat: left = haki/attack/skills/combo  |  right = weapon/instakill/movement
    Combat = {
        Haki   = TB.Combat.Left.CombatBox:AddTab("Haki"),
        Attack = TB.Combat.Left.CombatBox:AddTab("Attack"),
        Skills = TB.Combat.Left.CombatBox:AddTab("Skills"),
        Combo  = TB.Combat.Left.CombatBox:AddTab("Combo"),
    },
    Weapon = {
        Config    = TB.Combat.Right.WeaponBox:AddTab("Weapon"),
        InstaKill = TB.Combat.Right.WeaponBox:AddTab("Insta-Kill"),
        Movement  = TB.Combat.Right.WeaponBox:AddTab("Movement"),
    },
    -- Automation: left = stats/rolls  |  right = enchant/passive/config
    Stats = {
        Level     = TB.Automation.Left.StatsBox:AddTab("Stats"),
        Gem       = TB.Automation.Left.StatsBox:AddTab("Gem"),
        SkillMisc = TB.Automation.Left.StatsBox:AddTab("Misc"),
    },
    Rolls = {
        Reroll = TB.Automation.Left.RollsBox:AddTab("Rolls"),
        Ascend = TB.Automation.Left.RollsBox:AddTab("Ascend"),
        Trade  = TB.Automation.Left.RollsBox:AddTab("Trade"),
    },
    Enchant = {
        Equip         = TB.Automation.Right.EnchantBox:AddTab("Enchant"),
        Passive       = TB.Automation.Right.EnchantBox:AddTab("Passive"),
        PassiveConfig = TB.Automation.Right.EnchantBox:AddTab("Config"),
    },
    -- Dungeon: join/farm+replay/tower
    Dungeon = {
        Join  = TB.Dungeon.Left.DungeonBox:AddTab("Auto Join"),
        Farm  = TB.Dungeon.Left.DungeonBox:AddTab("Farm"),
        Tower = TB.Dungeon.Left.DungeonBox:AddTab("Tower"),
    },
    -- Teleport: island/quest/misc  |  moveset/mastery
    Waypoint = {
        Island  = TB.Teleport.Left.WaypointBox:AddTab("Island"),
        Quest   = TB.Teleport.Left.WaypointBox:AddTab("Quest"),
        MiscNPC = TB.Teleport.Left.WaypointBox:AddTab("Misc"),
    },
    NPCs = {
        Moveset = TB.Teleport.Right.NPCBox:AddTab("Moveset"),
        Mastery = TB.Teleport.Right.NPCBox:AddTab("Mastery"),
    },
    -- Misc: merchant/valentine/items/notify  |  puzzles/questlines
    MiscItems = {
        Merchant  = TB.Misc.Left.MiscLeftBox:AddTab("Merchant"),
        Valentine = TB.Misc.Left.MiscLeftBox:AddTab("Valentine"),
        Items     = TB.Misc.Left.MiscLeftBox:AddTab("Items"),
        Notify    = TB.Misc.Left.MiscLeftBox:AddTab("Notify"),
    },
    Puzzle = {
        Puzzles    = TB.Misc.Right.PuzzleBox:AddTab("Puzzles"),
        Questlines = TB.Misc.Right.PuzzleBox:AddTab("Questlines"),
    },
}

-- Backwards-compat aliases used by toggle callbacks throughout the file
TB_Tabs.Autofarm        = { T1 = TB_Tabs.Farm.Mob,       T4 = TB_Tabs.Weapon.Movement  }
TB_Tabs.BossMain        = { T1 = TB_Tabs.Farm.WorldBoss, T2 = TB_Tabs.Farm.Summon,
                             T3 = TB_Tabs.Farm.Pity,      T4 = TB_Tabs.Farm.Alt         }
TB_Tabs.MiscAuto        = { T1 = TB_Tabs.Combat.Haki,    T2 = TB_Tabs.Combat.Attack,
                             T3 = TB_Tabs.Combat.Combo                                  }
TB_Tabs.MiscAuto_Left   = { T1 = TB_Tabs.Rolls.Ascend,   T2 = TB_Tabs.Rolls.Reroll,
                             T3 = TB_Tabs.Rolls.Trade                                   }
TB_Tabs.Stats1          = { T1 = TB_Tabs.Stats.Level,    T2 = TB_Tabs.Stats.Gem,
                             T3 = TB_Tabs.Stats.SkillMisc                               }

-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: OVERVIEW
-- Left : Session info | Actions
-- Right: Priority config | Info/links
-- ══════════════════════════════════════════════════════════════════════════════
...
-- Session
TierLabel = GB.Overview.Left.Session:AddLabel("<b>Plan:</b> Loading...")
TimeLabel = GB.Overview.Left.Session:AddLabel("<b>Expires:</b> Loading...")
GB.Overview.Left.Session:AddLabel(
...
-- Actions
GB.Overview.Left.Actions:AddButton("Redeem All Codes", function()
...
GB.Overview.Left.Actions:AddButton({
...
-- Priority (right side of Overview)
for i = 1, #PriorityTasks do
    GB.Overview.Right.Priority:AddDropdown("SelectedPriority_" .. i, {
        Text      = "Priority " .. i,
...
-- Info
GB.Overview.Right.Info:AddLabel("⚠️ Some features require specific executor functions to work.", true)

-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: FARM
-- Left : Mob | World Boss | Summon | Pity | Alt Help
-- Right: Title switch | Rune switch | Build switch
-- ══════════════════════════════════════════════════════════════════════════════

-- ── Mob Farm ─────────────────────────────────────────────────────────────────
TB_Tabs.Farm.Mob:AddDropdown("SelectedMob", {
    Text = "Select Mob (s)", Values = Tables.MobList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Farm.Mob:AddButton("Refresh", UpdateNPCLists)
TB_Tabs.Farm.Mob:AddToggle("MobFarm",    { Text = "Autofarm Selected Mob", Default = false })
TB_Tabs.Farm.Mob:AddToggle("AllMobFarm", { Text = "Autofarm All Mobs",     Default = false })
TB_Tabs.Farm.Mob:AddDropdown("AllMobType", {
    Text = "Select Type [All Mob]", Values = {"Normal", "Fast"},
    Default = "Normal", Multi = false, Searchable = true,
})
TB_Tabs.Farm.Mob:AddToggle("LevelFarm", { Text = "Autofarm Level", Default = false })

-- ── World Boss ────────────────────────────────────────────────────────────────
TB_Tabs.Farm.WorldBoss:AddDropdown("SelectedBosses", {
    Text = "Select Bosses", Values = Tables.BossList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Farm.WorldBoss:AddToggle("BossesFarm",    { Text = "Autofarm Selected Boss", Default = false })
TB_Tabs.Farm.WorldBoss:AddToggle("AllBossesFarm", { Text = "Autofarm All Bosses",    Default = false })
TB_Tabs.Farm.WorldBoss:AddDropdown("ExceptBosses", {
    Text = "Select Except Bosses", Values = Tables.BossList,
    Default = nil, Multi = true, Searchable = true,
})

-- ── Summon ────────────────────────────────────────────────────────────────────
TB_Tabs.Farm.Summon:AddDropdown("SelectedSummon", {
    Text = "Select Summon Boss", Values = Tables.SummonList,
    Default = nil, Multi = false, Searchable = true,
})
TB_Tabs.Farm.Summon:AddDropdown("SelectedSummonDiff", {
    Text = "Select Difficulty", Values = Tables.DiffList,
    Default = nil, Multi = false, Searchable = true,
})
TB_Tabs.Farm.Summon:AddToggle("AutoSummon",     { Text = "Auto Summon",          Default = false })
TB_Tabs.Farm.Summon:AddToggle("SummonBossFarm", { Text = "Autofarm Summon Boss", Default = false })
TB_Tabs.Farm.Summon:AddDivider()
TB_Tabs.Farm.Summon:AddDropdown("SelectedOtherSummon", {
    Text = "Select Summon Boss (Other)", Values = Tables.OtherSummonList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Farm.Summon:AddDropdown("SelectedOtherSummonDiff", {
    Text = "Select Difficulty", Values = Tables.DiffList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Farm.Summon:AddToggle("AutoOtherSummon",  { Text = "Auto Summon",                 Default = false })
TB_Tabs.Farm.Summon:AddToggle("OtherSummonFarm",  { Text = "Autofarm Summon Boss (Other)", Default = false })

-- ── Pity ─────────────────────────────────────────────────────────────────────
PityLabel = TB_Tabs.Farm.Pity:AddLabel("<b>Pity:</b> 0/25")
TB_Tabs.Farm.Pity:AddLabel(
    "- Build Pity: Farm Boss Build Pity\n- Use Pity: Boss That Need To Farm\n- World boss Auto Spawn, Summon Boss",
    true
)
TB_Tabs.Farm.Pity:AddDropdown("SelectedBuildPity", {
    Text = "Select Boss [Build Pity]", Values = Tables.AllBossList,
    Default = nil, Multi = true, AllowNull = true, Searchable = true,
})
TB_Tabs.Farm.Pity:AddDropdown("SelectedUsePity", {
    Text = "Select Boss [Use Pity]", Values = Tables.AllBossList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Farm.Pity:AddDropdown("SelectedPityDiff", {
    Text = "Select Difficulty [Use Pity]", Values = Tables.DiffList,
    Default = nil, Multi = false, Searchable = true,
})
TB_Tabs.Farm.Pity:AddToggle("PityBossFarm", { Text = "Autofarm Pity Boss", Default = false })

-- ── Alt Help ──────────────────────────────────────────────────────────────────
TB_Tabs.Farm.Alt:AddDropdown("SelectedAltBoss", {
    Text = "Select Boss", Values = Tables.AllBossList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Farm.Alt:AddDropdown("SelectedAltDiff", {
    Text = "Select Difficulty", Values = Tables.DiffList,
    Default = nil, Multi = false, Searchable = true,
})
for i = 1, 5 do
    TB_Tabs.Farm.Alt:AddDropdown("SelectedAlt_" .. i, {
        Text             = "Select Alt #" .. i,
        SpecialType      = "Player",
...
TB_Tabs.Farm.Alt:AddToggle("AltBossFarm", { Text = "Auto Help Alt", Default = false })

-- ── Switch Groups (right side) ────────────────────────────────────────────────
CreateSwitchGroup(TB_Tabs.Switch.Title, "Title", "Title", CombinedTitleList)
CreateSwitchGroup(TB_Tabs.Switch.Rune,  "Rune",  "Rune",  Tables.RuneList)
CreateSwitchGroup(TB_Tabs.Switch.Build, "Build", "Build", Tables.BuildList)

-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: COMBAT
-- Left : Haki | Attack | Skills | Combo
-- Right: Weapon Config | Insta-Kill | Movement
-- ══════════════════════════════════════════════════════════════════════════════

-- ── Haki ─────────────────────────────────────────────────────────────────────
TB_Tabs.Combat.Haki:AddToggle("ObserHaki",     { Text = "Auto Observation Haki", Default = false })
TB_Tabs.Combat.Haki:AddToggle("ArmHaki",       { Text = "Auto Armament Haki",    Default = false })
TB_Tabs.Combat.Haki:AddToggle("ConquerorHaki", { Text = "Auto Conqueror Haki",   Default = false })

-- ── Attack ────────────────────────────────────────────────────────────────────
TB_Tabs.Combat.Attack:AddLabel(
    "Autofarm already has <b>auto-M1 built in</b>.\n" ..
    "You do not need to enable this separately unless you have <b>any issues with the autofarm M1.</b>",
    true
)
TB_Tabs.Combat.Attack:AddToggle("AutoM1",   { Text = "Auto Attack", Default = false })
TB_Tabs.Combat.Attack:AddToggle("KillAura", { Text = "Kill Aura",   Default = false })
TB_Tabs.Combat.Attack:AddSlider("KillAuraCD",    { Text = "Cooldown", Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
TB_Tabs.Combat.Attack:AddSlider("KillAuraRange", { Text = "Range",    Default = 200, Min = 0,   Max = 200, Rounding = 0 })

-- ── Skills ────────────────────────────────────────────────────────────────────
TB_Tabs.Combat.Skills:AddLabel(
    "Mode:\n- <b>Normal:</b> Check skill cooldowns\n- <b>Instant:</b> No check (may affect performance when use in long time.)",
    true
)
TB_Tabs.Combat.Skills:AddDropdown("SelectedSkills", {
    Text = "Select Skills", Values = {"Z", "X", "C", "V", "F"},
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Combat.Skills:AddDropdown("AutoSkillType", {
    Text = "Select Mode", Values = {"Normal", "Instant"},
    Default = "Normal", Multi = false, Searchable = true,
})
TB_Tabs.Combat.Skills:AddToggle("OnlyTarget",         { Text = "Target Only",      Default = false })
TB_Tabs.Combat.Skills:AddToggle("AutoSkill_BossOnly", { Text = "Use On Boss Only", Default = false })
TB_Tabs.Combat.Skills:AddSlider("AutoSkill_BossHP", {
    Text = "Boss HP%", Default = 100, Min = 1, Max = 100, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Combat.Skills:AddToggle("AutoSkill", { Text = "Auto Use Skills", Default = false })

-- ── Combo ─────────────────────────────────────────────────────────────────────
TB_Tabs.Combat.Combo:AddLabel("Example:\n- Z > X > C > 0.5 > V\n- Z, X, C, 0.5, V", true)
TB_Tabs.Combat.Combo:AddLabel("Mode:\n- Normal: Wait for input CD (ex: Z > 0.5)\n- Instant: Ignore input CD", true)
TB_Tabs.Combat.Combo:AddInput("ComboPattern", {
    Text        = "Combo Pattern",
    Default     = "Z > X > C > V > F",
    Placeholder = "combo..",
})
TB_Tabs.Combat.Combo:AddDropdown("ComboMode", {
    Text = "Select Mode", Values = {"Normal", "Instant"}, Default = "Normal",
})
TB_Tabs.Combat.Combo:AddToggle("ComboBossOnly", { Text = "Boss Only", Default = false })
TB_Tabs.Combat.Combo:AddToggle("AutoCombo", {
...
-- ── Weapon Config (right side) ────────────────────────────────────────────────
TB_Tabs.Weapon.Config:AddDropdown("SelectedWeaponType", {
    Text = "Select Weapon Type", Values = Tables.Weapon,
    Default = nil, Multi = true,
})
TB_Tabs.Weapon.Config:AddSlider("SwitchWeaponCD", {
    Text = "Switch Weapon Delay", Default = 4, Min = 1, Max = 20, Rounding = 0,
    Callback = function(a) tonumber(a) end
})

-- ── Insta-Kill ────────────────────────────────────────────────────────────────
TB_Tabs.Weapon.InstaKill:AddToggle("InstaKill", { Text = "Instant Kill", Default = false })
TB_Tabs.Weapon.InstaKill:AddDropdown("InstaKillType", {
    Text = "Select Type", Values = {"V1", "V2"},
    Default = "V1", Multi = false, Searchable = true,
})
TB_Tabs.Weapon.InstaKill:AddSlider("InstaKillHP", {
    Text = "HP% For Insta-Kill", Default = 90, Min = 1, Max = 100, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.InstaKill:AddInput("InstaKillMinHP", {
...
-- ── Movement ──────────────────────────────────────────────────────────────────
TB_Tabs.Weapon.Movement:AddToggle("IslandTP", { Text = "Island TP [Autofarm]", Default = true })
TB_Tabs.Weapon.Movement:AddSlider("IslandTPCD", {
    Text = "Island TP CD", Default = 0.67, Min = 0, Max = 2.5, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.Movement:AddSlider("TargetTPCD", {
    Text = "Target TP CD", Default = 0, Min = 0, Max = 5, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.Movement:AddSlider("TargetDistTP", {
    Text = "Target Distance TP [Tween]", Default = 0, Min = 0, Max = 100, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.Movement:AddSlider("M1Speed", {
    Text = "M1 Attack Cooldown", Default = 0.2, Min = 0, Max = 1, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.Movement:AddDropdown("SelectedMovementType", {
    Text = "Select Movement Type", Values = {"Teleport", "Tween"},
    Default = "Tween", Multi = false, Searchable = true,
})
TB_Tabs.Weapon.Movement:AddDropdown("SelectedFarmType", {
    Text = "Select Farm Type", Values = {"Behind", "Above", "Below"},
    Default = "Behind", Multi = false, Searchable = true,
})
TB_Tabs.Weapon.Movement:AddSlider("Distance", {
    Text = "Farm Distance", Default = 12, Min = 0, Max = 30, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Weapon.Movement:AddSlider("TweenSpeed", {
    Text = "Tween Speed", Default = 160, Min = 0, Max = 500, Rounding = 0,
    Callback = function(a) tonumber(a) end
})

-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: AUTOMATION
-- Left : Stats (Stats/Gem/Misc) | Rolls (Rolls/Ascend/Trade)
-- Right: Enchant (Enchant+Blessing / Passive / Config)
-- ══════════════════════════════════════════════════════════════════════════════

-- ── Stats → Level ─────────────────────────────────────────────────────────────
TB_Tabs.Stats.Level:AddDropdown("SelectedStats", {
    Text = "Select Stat (s)", Values = {"Melee", "Defense", "Sword", "Power"},
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Stats.Level:AddToggle("AutoStats", { Text = "Auto UP Stats", Default = false })

-- ── Stats → Gem ───────────────────────────────────────────────────────────────
TB_Tabs.Stats.Gem:AddLabel(
    "- ⚠️ Increase delay based on your ping/internet speed.\n- ⚠️ Low delay settings are not recommended.\n- Reroll once for this to work.",
    true
)
StatsLabel = TB_Tabs.Stats.Gem:AddLabel("N/A", true)
TB_Tabs.Stats.Gem:AddDropdown("SelectedGemStats", {
    Text = "Select Stat (s)", Values = Tables.GemStat,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Stats.Gem:AddDropdown("SelectedRank", {
    Text = "Select Rank (s)", Values = Tables.GemRank,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Stats.Gem:AddSlider("StatsRollCD", {
    Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Stats.Gem:AddToggle("AutoRollStats", { Text = "Auto Roll Stats", Default = false })

-- ── Stats → Misc ──────────────────────────────────────────────────────────────
TB_Tabs.Stats.SkillMisc:AddToggle("AutoSkillTree",     { Text = "Auto Skill Tree",         Default = false })
TB_Tabs.Stats.SkillMisc:AddToggle("ArtifactMilestone", { Text = "Auto Artifact Milestone", Default = false })

-- ── Rolls → Reroll ────────────────────────────────────────────────────────────
TB_Tabs.Rolls.Reroll:AddLabel(
    "- ⚠️ Increase delay based on your ping/internet speed.\n- ⚠️ Low delay settings are not recommended.",
    true
)
TB_Tabs.Rolls.Reroll:AddSlider("RollCD", {
    Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Rolls.Reroll:AddDropdown("SelectedTrait", {
    Text = "Select Trait (s)", Values = Tables.TraitList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Rolls.Reroll:AddToggle("AutoTrait", { Text = "Auto Roll Trait", Default = false })
TB_Tabs.Rolls.Reroll:AddDropdown("SelectedRace", {
    Text = "Select Race (s)", Values = Tables.RaceList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Rolls.Reroll:AddToggle("AutoRace", { Text = "Auto Roll Race", Default = false })
TB_Tabs.Rolls.Reroll:AddDropdown("SelectedClan", {
    Text = "Select Clan (s)", Values = Tables.ClanList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Rolls.Reroll:AddToggle("AutoClan", { Text = "Auto Roll Clan", Default = false })

-- ── Rolls → Ascend ────────────────────────────────────────────────────────────
TB_Tabs.Rolls.Ascend:AddToggle("AutoAscend", { Text = "Auto Ascend", Default = false })
for i = 1, 10 do
    Tables.AscendLabels[i] = TB_Tabs.Rolls.Ascend:AddLabel("", true)
    Tables.AscendLabels[i]:SetVisible(false)
end

-- ── Rolls → Trade ─────────────────────────────────────────────────────────────
TB_Tabs.Rolls.Trade:AddDropdown("SelectedTradePlr", {
    Text              = "Select Player",
    SpecialType       = "Player",
    ExcludeLocalPlayer = true,
    AllowNull         = true,
...
TB_Tabs.Rolls.Trade:AddDropdown("SelectedTradeItems", {
    Text = "Select Item (s)", Values = Tables.OwnedItem,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Rolls.Trade:AddToggle("ReqTrade",       { Text = "Auto Send Request",   Default = false })
TB_Tabs.Rolls.Trade:AddToggle("ReqTradeAccept", { Text = "Auto Accept Request", Default = false })
TB_Tabs.Rolls.Trade:AddToggle("AutoAccept",     { Text = "Auto Accept Trade",   Default = false })

-- ── Enchant → Equip (Enchant + Blessing) ──────────────────────────────────────
TB_Tabs.Enchant.Equip:AddDropdown("SelectedEnchant", {
    Text = "Select Enchant", Values = Tables.OwnedAccessory,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Enchant.Equip:AddToggle("AutoEnchant",    { Text = "Auto Enchant",     Default = false })
TB_Tabs.Enchant.Equip:AddToggle("AutoEnchantAll", { Text = "Auto Enchant All", Default = false })
TB_Tabs.Enchant.Equip:AddDivider()
TB_Tabs.Enchant.Equip:AddDropdown("SelectedBlessing", {
    Text = "Select Blessing", Values = Tables.OwnedWeapon,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Enchant.Equip:AddToggle("AutoBlessing",    { Text = "Auto Blessing",     Default = false })
TB_Tabs.Enchant.Equip:AddToggle("AutoBlessingAll", { Text = "Auto Blessing All", Default = false })

-- ── Enchant → Passive ─────────────────────────────────────────────────────────
SpecPassiveLabel = TB_Tabs.Enchant.Passive:AddLabel("N/A", true)
TB_Tabs.Enchant.Passive:AddDropdown("SelectedPassive", {
    Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Enchant.Passive:AddDropdown("SelectedSpec", {
    Text = "Target Passives", Values = Tables.SpecPassive,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.Enchant.Passive:AddSlider("SpecRollCD", {
    Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2,
    Callback = function(a) tonumber(a) end
})
TB_Tabs.Enchant.Passive:AddToggle("AutoSpec", { Text = "Auto Reroll Passive", Default = false })

-- ── Enchant → Config (dynamic passive sliders) ────────────────────────────────
TB_Tabs.Enchant.PassiveConfig:AddLabel(
    "- ⚠️ <b>Only adjust these values if you are an advanced user.\nWill stop once the selected passive reaches your set minimum value.</b>",
    true
)
            ...
            if data and data.Buffs then
                for statKey, range in pairs(data.Buffs) do
                    ...
                    if not Options[sliderId] then
                        ...
                        Shared.SpecStatsSlider[sliderId] = TB_Tabs.Enchant.PassiveConfig:AddSlider(sliderId, {
                            Text    = label,
                            Default = minVal,
                            Min     = minVal,
                            Max     = maxVal,
...
-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: ARTIFACT
-- Left : Status | Upgrade | Auto-Equip
-- Right: Lock | Delete
-- ══════════════════════════════════════════════════════════════════════════════
...
GB.Artifact.Left.Upgrade:AddSlider("UpgradeLimit", {
    Text = "Upgrade Limit", Default = 0, Min = 0, Max = 15, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
GB.Artifact.Left.Upgrade:AddDropdown("Up_MS", {
    Text = "Main Stat Filter", Values = {}, Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Left.Upgrade:AddToggle("ArtifactUpgrade", { Text = "Auto Upgrade",      Default = false })
GB.Artifact.Left.Upgrade:AddToggle("UpgradeStage",    { Text = "Upgrade in Stages", Default = false })

-- Auto-Equip
GB.Artifact.Left.Equip:AddDropdown("Eq_Type", {
    Text = "Artifact Type", Values = {}, Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Left.Equip:AddDropdown("Eq_MS", {
    Text = "Main Stat Filter", Values = {}, Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Left.Equip:AddDropdown("Eq_SS", {
    Text = "Sub Stat Filter", Values = {}, Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Left.Equip:AddToggle("ArtifactEquip", { Text = "Auto Equip", Default = false })
...
GB.Artifact.Right.Lock:AddDropdown("Lock_Set", {
    Text = "Artifact Set", Values = {}, Default = nil, Multi = true, Searchable = true,
})
...
GB.Artifact.Right.Lock:AddSlider("Lock_MinSS", {
    Text = "Min Sub-Stats", Default = 0, Min = 0, Max = 4, Rounding = 0,
    Callback = function(a) tonumber(a) end
})
GB.Artifact.Right.Lock:AddToggle("ArtifactLock", { Text = "Auto Lock", Default = false })
...
GB.Artifact.Right.Delete:AddDropdown("Del_MS_Helmet", {
    Text = "Main Stat [Helmet]", Values = {"FlatDefense", "Defense"},
    Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Right.Delete:AddDropdown("Del_MS_Gloves", {
    Text = "Main Stat [Gloves]", Values = {"Damage"},
    Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Right.Delete:AddDropdown("Del_MS_Body", {
    Text = "Main Stat [Body]", Values = {}, Default = nil, Multi = true, Searchable = true,
})
GB.Artifact.Right.Delete:AddDropdown("Del_MS_Boots", {
    Text = "Main Stat [Boots]", Values = {}, Default = nil, Multi = true, Searchable = true,
})
...
GB.Artifact.Right.Delete:AddToggle("ArtifactDelete", { Text = "Auto Delete",          Default = false })
GB.Artifact.Right.Delete:AddToggle("DeleteUnlock",   { Text = "Auto Delete Unlocked", Default = false })
...
-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: DUNGEON
-- Left : Auto Join | Farm + Replay | Infinity Tower
-- ══════════════════════════════════════════════════════════════════════════════

-- Auto Join
TB_Tabs.Dungeon.Join:AddLabel("BossRush Supported.", true)
TB_Tabs.Dungeon.Join:AddDropdown("SelectedDungeon", {
    Text = "Select Dungeon", Values = Tables.DungeonList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Dungeon.Join:AddToggle("AutoDungeon", { Text = "Auto Join Dungeon", Default = false })

-- Farm + Replay
DungeonCount = TB_Tabs.Dungeon.Farm:AddLabel("Dungeon Completed: N/A")
TB_Tabs.Dungeon.Farm:AddDropdown("SelectedDiff", {
    Text = "Select Difficulty", Values = {"Easy", "Medium", "Hard", "Extreme"},
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Dungeon.Farm:AddToggle("AutoDiff",   { Text = "Auto Select Difficulty", Default = false })
TB_Tabs.Dungeon.Farm:AddToggle("AutoReplay", { Text = "Auto Replay",            Default = false })
TB_Tabs.Dungeon.Farm:AddDivider()
TB_Tabs.Dungeon.Farm:AddLabel("Auto Replay at Floor", true)
TB_Tabs.Dungeon.Farm:AddToggle("AutoReplayAtFloor", { Text = "Auto Replay at Floor", Default = false })
TB_Tabs.Dungeon.Farm:AddSlider("ReplayFloorValue", {
    Text = "Target Floor", Default = 10, Min = 1, Max = 200, Rounding = 0, Compact = true,
})
TB_Tabs.Dungeon.Farm:AddToggle("DungeonAutofarm", { Text = "Autofarm Dungeon", Default = false })

-- Infinity Tower
TB_Tabs.Dungeon.Tower:AddLabel("Infinity Tower / Proximity Farm", true)
TB_Tabs.Dungeon.Tower:AddSlider("ITRange", {
    Text = "Scan Range", Default = 400, Min = 50, Max = 1000, Rounding = 0,
})
TB_Tabs.Dungeon.Tower:AddToggle("AutoInfinityTower", {
    Text = "Auto Infinity Tower / Nearby Farm", Default = false,
})

-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: PLAYER
-- Left : General movement | Server utilities
-- Right: Game visuals     | Safety / kick
-- ══════════════════════════════════════════════════════════════════════════════
...
GB.Player.Left.Server:AddButton({ Text = "Serverhop", Func = function()
    task.spawn(function()
        ...
        if #candidates == 0 then Library:Notify("Serverhop: No available server found", 3) return end
...
-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: TELEPORT
-- Left : Island | Quest NPC | Misc NPC
-- Right: Moveset NPC | Mastery NPC
-- ══════════════════════════════════════════════════════════════════════════════

TB_Tabs.Waypoint.Island:AddDropdown("SelectedIsland", {
    Text      = "Select Island",
...
TB_Tabs.Waypoint.Quest:AddDropdown("SelectedQuestNPC", {
    Text      = "Select NPC",
...
TB_Tabs.Waypoint.MiscNPC:AddDropdown("SelectedMiscNPC", {
...
TB_Tabs.Waypoint.MiscNPC:AddDropdown("SelectedMiscAllNPC", {
...
TB_Tabs.NPCs.Moveset:AddDropdown("SelectedMovesetNPC", {
    Text      = "Select NPC",
...
TB_Tabs.NPCs.Mastery:AddDropdown("SelectedMasteryNPC", {
    Text      = "Select NPC",
...
-- ══════════════════════════════════════════════════════════════════════════════
-- TAB: MISC
-- Left : Merchant | Valentine | Items (Chests+Craft) | Notify
-- Right: Puzzles  | Questlines
-- ══════════════════════════════════════════════════════════════════════════════

-- ── Merchant (Regular) ────────────────────────────────────────────────────────
MerchantTimerLabel = TB_Tabs.MiscItems.Merchant:AddLabel("Refresh: N/A")
TB_Tabs.MiscItems.Merchant:AddDropdown("SelectedMerchantItems", {
    Text = "Select Item (s)", Values = Tables.MerchantList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.MiscItems.Merchant:AddToggle("AutoMerchant", { Text = "Auto Buy Selected Items", Default = false })

-- ── Valentine Merchant ────────────────────────────────────────────────────────
TB_Tabs.MiscItems.Valentine:AddLabel("Able to causing performance issues.", true)
ValentineLabel = TB_Tabs.MiscItems.Valentine:AddLabel("Hearts: N/A")
TB_Tabs.MiscItems.Valentine:AddDropdown("SelectedValentineMerchantItems", {
    Text = "Select Item (s)", Values = Tables.ValentineMerchantList,
    Default = nil, Multi = true, Searchable = true,
})
ValentinePriceLabel = TB_Tabs.MiscItems.Valentine:AddLabel("Price: N/A")
TB_Tabs.MiscItems.Valentine:AddToggle("AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

-- ── Items (Chests + Craft) ────────────────────────────────────────────────────
TB_Tabs.MiscItems.Items:AddDropdown("SelectedChests", {
    Text = "Select Chest (s)", Values = Tables.Rarities,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.MiscItems.Items:AddToggle("AutoChest", { Text = "Auto Open Chest", Default = false })
TB_Tabs.MiscItems.Items:AddDivider()
TB_Tabs.MiscItems.Items:AddDropdown("SelectedCraftItems", {
    Text = "Select Item (s) To Craft", Values = Tables.CraftItemList,
    Default = nil, Multi = true, Searchable = true,
})
TB_Tabs.MiscItems.Items:AddToggle("AutoCraftItem", { Text = "Auto Craft Item", Default = false })

-- ── Notify ────────────────────────────────────────────────────────────────────
TB_Tabs.MiscItems.Notify:AddToggle("AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

-- ── Puzzles ───────────────────────────────────────────────────────────────────
TB_Tabs.Puzzle.Puzzles:AddButton({
...
TB_Tabs.Puzzle.Puzzles:AddButton({
    ...
    Disabled = not Support.Proximity,
    Func     = function() UniversalPuzzleSolver("Slime") end
})
TB_Tabs.Puzzle.Puzzles:AddButton({
...
TB_Tabs.Puzzle.Puzzles:AddButton({
    ...
    Disabled = not Support.Proximity,
    Func     = function() UniversalPuzzleSolver("Demonite") end
})
TB_Tabs.Puzzle.Puzzles:AddButton({
...
-- ── Questlines ────────────────────────────────────────────────────────────────
TB_Tabs.Puzzle.Questlines:AddLabel({
...
TB_Tabs.Puzzle.Questlines:AddDropdown("SelectedQuestline", {
    Text = "Select Questline", Values = Tables.QuestlineList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Puzzle.Questlines:AddDropdown("SelectedQuestline_Player", {
    Text              = "Select Player",
    SpecialType       = "Player",
    ExcludeLocalPlayer = true,
    AllowNull         = true,
...
TB_Tabs.Puzzle.Questlines:AddDropdown("SelectedQuestline_DMGTaken", {
    Text = "Select Mob [Take Damage]", Values = Tables.AllEntitiesList,
    Default = nil, Multi = false, AllowNull = true, Searchable = true,
})
TB_Tabs.Puzzle.Questlines:AddButton("Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)
TB_Tabs.Puzzle.Questlines:AddToggle("AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })
...
GB.Webhook.Left.Config:AddToggle("PingUser",    { Text = "Ping User",    Default = false })
GB.Webhook.Left.Config:AddToggle("SendWebhook", {
    Text = "Send Webhook", Default = false, Disabled = not Support.Webhook,
})
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SAVE MANAGER / THEME MANAGER SETUP
-- ══════════════════════════════════════════════════════════════════════════════

ThemeManager:SetLibrary(Library)
SaveManager:SetLibrary(Library)
SaveManager:IgnoreThemeSettings()
SaveManager:SetIgnoreIndexes({ "SelectedIsland", "SelectedQuestNPC", "SelectedMiscNPC", "SelectedMiscAllNPC", "SelectedMovesetNPC", "SelectedMasteryNPC" })
...
-- ══════════════════════════════════════════════════════════════════════════════
-- POST-UI INITIALISATION  (dropdown value injection, NPC lists, etc.)
-- ══════════════════════════════════════════════════════════════════════════════
...
Options.SelectedTrait:OnChanged(function() SyncTraitAutoSkip() end)
Options.SelectedRace:OnChanged(function()  SyncRaceSettings()  end)
Options.SelectedClan:OnChanged(function()  SyncClanSettings()  end)
...
Options.SelectedSpec:OnChanged(function()    SyncSpecPassiveAutoSkip() end)
Options.SelectedPassive:OnChanged(function() UpdateSpecPassiveLabel()  end)
...
Toggles.AutoReplay:OnChanged(function(state)
    if not state then Shared.DungeonReplaying = false end
end)
Toggles.AutoDiff:OnChanged(function() end)
...
Options.SelectedKickType:OnChanged(function() CheckServerTypeSafety() end)
...
Toggles.FPSBoost_AF:OnChanged(function(state) if state then ApplyIslandWipe() end end)
...
Options.PanicKeybind:OnClick(function() PanicStop() end)
    ...
    while Toggles.AutoServerhop.Value do
        ...
        if tick() - Shared._HopStart >= mins * 60 then
            ...
            task.spawn(function()
                ...
                if not ok then Library:Notify("AutoServerhop: HTTP request failed", 3) return end
                local decoded
                pcall(function() decoded = HttpService:JSONDecode(raw) end)
                if not decoded or not decoded.data then Library:Notify("AutoServerhop: No server data", 3) return end
                local candidates = {}
                ...
                if #candidates == 0 then Library:Notify("AutoServerhop: No available servers found", 3) return end
                local picked = candidates[math.random(1, math.min(5, #candidates))]
    ...
    while Toggles.AutoReplayAtFloor.Value do
        ...
        local ok, currentFloor = pcall(function()
            ...
            local label = dungeonUI
                ...
                and dungeonUI.ContentFrame:FindFirstChild("WaveFrame")
                and dungeonUI.ContentFrame.WaveFrame:FindFirstChild("Holder")
            ...
            local num = text:match("^%s*%a*%s*(%d+)") or text:match("(%d+)")
            return num and tonumber(num) or nil
        end)
        if not ok or not currentFloor then continue end
        if currentFloor >= targetFloor then
            Library:Notify(
                string.format("[AutoReplay] Floor %d >= target %d — dying to replay!", currentFloor, targetFloor), 4
            )
            task.wait(0.3)
            local char = GetCharacter()
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
...
-- ══════════════════════════════════════════════════════════════════════════════
-- BACKGROUND LOOPS (inventory sync, pity label, anti-AFK, main farm, recovery)
-- ══════════════════════════════════════════════════════════════════════════════
...
task.spawn(function()
    while true do
        ...
        if not success then Library:Notify("ERROR: " .. tostring(err), 10) end
    end
end)

-- Target resolution + summon dispatch loop
task.spawn(function()
    ...
    while task.wait() do
        ...
        if Shared.TargetValid and (not Shared.Target or not Shared.Target.Parent or (function()
            ...
            if Shared.Target:FindFirstChild("IK_Active") then return false end
        ...
        local hasPityBoss = Options.SelectedUsePity.Value
            and Options.SelectedUsePity.Value ~= ""
            and Options.SelectedUsePity.Value ~= "None"
