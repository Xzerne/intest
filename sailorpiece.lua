...
local SeaFarm = {
    SavedCFrame  = nil,   -- CFrame lưu bởi người dùng
    DeathConn    = nil,   -- connection theo dõi Humanoid.Died
    CurrentBoss  = nil,   -- boss model đang nhắm vào
    StatusLabel  = nil,   -- label UI (gán sau khi tạo UI)
    LastNotified = 0,     -- throttle notify
}

local SEA_BOSS_KEYWORDS = {
    "SeaSerpent", "Kraken", "SeaSerpentBoss", "KrakenBoss"
}

local function SetSeaFarmStatus(txt)
    if SeaFarm.StatusLabel then
        pcall(function() SeaFarm.StatusLabel:SetText(txt) end)
    end
end

local function NotifySeaFarm(msg, dur)
    local now = tick()
    if now - SeaFarm.LastNotified > 3 then
        SeaFarm.LastNotified = now
        Library:Notify("[Sea Farm] " .. msg, dur or 4)
    end
end

-- Tìm sea boss sống trong workspace.NPCs hoặc workspace root
local function FindSeaBoss()
    local selectedDict = Options.SeaBossTarget and Options.SeaBossTarget.Value or {}
    local hasSelection = false
    for _, v in pairs(selectedDict) do if v then hasSelection = true break end end

    local function matchBoss(name)
        local nm = name:lower()
        if hasSelection then
            for k, v in pairs(selectedDict) do
                if v and nm:find(k:lower(), 1, true) then return true end
            end
        else
            -- fallback: kiểm tra keyword mặc định
            for _, kw in ipairs(SEA_BOSS_KEYWORDS) do
    ...
    -- 1) workspace.NPCs
    for _, obj in ipairs(PATH.Mobs:GetChildren()) do
        if obj:IsA("Model") and matchBoss(obj.Name) then
            local hum = obj:FindFirstChildOfClass("Humanoid")
            if hum and hum.Health > 0 then return obj end
        end
    end
    -- 2) workspace root (một số game spawn boss ngoài NPCs folder)
...
-- Gắn lại death listener sau mỗi lần respawn
local function BindSeaDeath()
    if SeaFarm.DeathConn then
        pcall(function() SeaFarm.DeathConn:Disconnect() end)
        SeaFarm.DeathConn = nil
    end
    ...
    SeaFarm.DeathConn = hum.Died:Connect(function()
        SetSeaFarmStatus("Died – waiting respawn...")
        task.wait(3.5)

        -- Đợi character mới
        local newChar = Plr.Character
        if not newChar or not newChar:FindFirstChildOfClass("Humanoid") then
            local ok2, nc = pcall(function() return Plr.CharacterAdded:Wait() end)
            newChar = ok2 and nc or Plr.Character
        end

        local newRoot = newChar and newChar:WaitForChild("HumanoidRootPart", 6)
        if newRoot and SeaFarm.SavedCFrame then
            task.wait(0.6)
            newRoot.CFrame = SeaFarm.SavedCFrame
            NotifySeaFarm("Respawned → returned to saved position.", 4)
            SetSeaFarmStatus("Idle – waiting for sea boss...")
        end
...
local function Func_SeaFarm()
    if not SeaFarm.SavedCFrame then
        Library:Notify("[Sea Farm] Set a CFrame position first!", 4)
        Toggles.AutoSeaFarm:SetValue(false)
        return
    end

    BindSeaDeath()
    SetSeaFarmStatus("Active – scanning for sea boss...")

    local attackDist  = tonumber(Options.SeaFarmDist and Options.SeaFarmDist.Value) or 20
    local m1CD        = tonumber(Options.SeaFarmM1CD and Options.SeaFarmM1CD.Value) or 0.15
    local useSkills   = Toggles.SeaFarm_UseSkills and Toggles.SeaFarm_UseSkills.Value
    ...
    while Toggles.AutoSeaFarm.Value do
        attackDist = tonumber(Options.SeaFarmDist and Options.SeaFarmDist.Value) or 20
        m1CD       = tonumber(Options.SeaFarmM1CD and Options.SeaFarmM1CD.Value) or 0.15
        useSkills  = Toggles.SeaFarm_UseSkills and Toggles.SeaFarm_UseSkills.Value

        local boss = FindSeaBoss()

        if boss and boss.Parent then
            SeaFarm.CurrentBoss = boss
            SetSeaFarmStatus("Attacking: " .. boss.Name)

            local char = GetCharacter()
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if not root then task.wait(0.3) continue end

            -- Đứng ở góc nhìn hướng boss, cách attackDist
            local bossPos  = boss:GetPivot().Position
            local offset   = Options.SeaFarmPosType and Options.SeaFarmPosType.Value or "Front"
            local attackPos

            if offset == "Above" then
                attackPos = bossPos + Vector3.new(0, attackDist, 0)
            elseif offset == "Below" then
                attackPos = bossPos + Vector3.new(0, -attackDist, 0)
            else -- Front
                local dir = (root.Position - bossPos)
                local horizontal = Vector3.new(dir.X, 0, dir.Z)
                if horizontal.Magnitude < 0.1 then horizontal = Vector3.new(0, 0, 1) end
                attackPos = bossPos + horizontal.Unit * attackDist + Vector3.new(0, 5, 0)
            end

            local moveDist = (root.Position - attackPos).Magnitude
            if moveDist > attackDist * 1.5 then
                root.CFrame = CFrame.lookAt(attackPos, bossPos)
                root.AssemblyLinearVelocity  = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
                task.wait(0.08)
            end
            ...
            pcall(function() Remotes.M1:FireServer(bossPos) end)

            -- Skills nếu bật
            if useSkills and tool then
                ...
                local skillDelay = tonumber(Options.AutoSkill_Delay and Options.AutoSkill_Delay.Value) or 0.1
            ...
            -- Không có boss → về saved CFrame chờ
            SeaFarm.CurrentBoss = nil
            SetSeaFarmStatus("Idle – no sea boss found, waiting...")

            local char = GetCharacter()
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root and SeaFarm.SavedCFrame then
                local distToSaved = (root.Position - SeaFarm.SavedCFrame.Position).Magnitude
                if distToSaved > 60 then
                    root.CFrame = SeaFarm.SavedCFrame
                    root.AssemblyLinearVelocity  = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero
                end
            end
...
local function Func_AutoSkill()
    ...
    while task.wait() do
        ...
        -- ── Self-HP guard ──────────────────────────────────────────────────
        local selfHPThresh = tonumber(Options.AutoSkill_SelfHP and Options.AutoSkill_SelfHP.Value) or 0
        if selfHPThresh > 0 then
            local char = GetCharacter()
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            if hum and hum.MaxHealth > 0 then
                local pct = (hum.Health / hum.MaxHealth) * 100
                if pct < selfHPThresh then continue end
            end
        end
        ...
        -- ── Skill filter: only keys enabled in dropdown ───────────────────
        local activeKeys = {}
        ...
        if #activeKeys == 0 then task.wait(0.2) continue end

        -- inter-skill delay from slider (default 0.1 s)
        local skillDelay = tonumber(Options.AutoSkill_Delay and Options.AutoSkill_Delay.Value) or 0.1
        ...
        -- ── SMART BURST mode ─────────────────────────────────────────────
        -- Fires ALL ready skills at once, then waits until at least one is
        -- ready again before the next burst. Minimises empty-loop spinning.
        elseif useMode == "Smart Burst" then
            local mainFrame = PGui:FindFirstChild("CooldownUI") and PGui.CooldownUI:FindFirstChild("MainFrame")
            if not mainFrame then task.wait(0.2) continue end
            ...
            -- Wait until at least one key becomes ready again
            if firedAny then
                ...
                repeat
                    ...
                    for _, key in ipairs(activeKeys) do
                        if IsSkillReady(key) then anyReady = true break end
                    end
                    if anyReady then break end
                until tick() - waitStart > 30   -- safety timeout
            else
                task.wait(0.15)
            end

        -- ── NORMAL mode ───────────────────────────────────────────────────
        -- Checks cooldown per-skill, fires ready ones, small delay between
        else
            local mainFrame = PGui:FindFirstChild("CooldownUI") and PGui.CooldownUI:FindFirstChild("MainFrame")
            if not mainFrame then task.wait(0.2) continue end
...
local TB_Tabs = {
    ...
    -- Boss tab: World Boss / Summon / Pity / Alt / Sea
    BossMain = {
        ...
        T5 = TB.Boss.Left.BossMain:AddTab("Sea"),
    },
...
-- ── SEA FARM UI (T5) ──────────────────────────────────────────────────────
do
    local T5 = TB_Tabs.BossMain.T5

    T5:AddLabel("<b>Sea Boss Farm</b>\nAuto Farm Sea Serpent / Kraken.\Save Position First!", true)

    -- Status label
    local seaStatusLabel = T5:AddLabel("Status: Disabled")
    SeaFarm.StatusLabel = seaStatusLabel

    T5:AddDivider()

    -- Lưu / hiển thị CFrame
    local seaSavedLabel = T5:AddLabel("Saved Pos: None")

    T5:AddButton("Save Current Position", function()
        local char = GetCharacter()
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            SeaFarm.SavedCFrame = root.CFrame
            local p = root.CFrame.Position
            pcall(function()
                seaSavedLabel:SetText(string.format(
                    "Saved: (%.0f, %.0f, %.0f)", p.X, p.Y, p.Z
                ))
            end)
            Library:Notify("[Sea Farm] Position saved!", 3)
        else
            Library:Notify("[Sea Farm] No character!", 3)
        end
    end)

    T5:AddButton("Clear Saved Position", function()
        SeaFarm.SavedCFrame = nil
        pcall(function() seaSavedLabel:SetText("Saved Pos: None") end)
    end)

    T5:AddDivider()

    -- Chọn boss
    T5:AddDropdown("SeaBossTarget", {
        Text  = "Select Sea Boss(es)",
        Values = {"SeaSerpent", "Kraken", "SeaSerpentBoss", "KrakenBoss", "LeviathanBoss", "SeaKing", "OceanBoss"},
        Default = nil,
        Multi = true,
        Searchable = true,
    })
    ...
    -- Vị trí đánh
    T5:AddDropdown("SeaFarmPosType", {
        Text   = "Attack Position",
        Values = {"Front", "Above", "Below"},
        Default = "Front",
        Multi  = false,
    })

    -- Khoảng cách
    T5:AddSlider("SeaFarmDist", {
    ...
    -- Bật skills kèm theo
    T5:AddToggle("SeaFarm_UseSkills", {
        Text    = "Use Auto Skills",
        Default = true,
    })
...
TB_Tabs.MiscAuto.T2:AddDropdown("AutoSkillType", {
    Text = "Select Mode",
    Values = {"Normal", "Instant", "Smart Burst"},
    Default = "Normal",
    Multi = false,
    Searchable = true,
})

TB_Tabs.MiscAuto.T2:AddSlider("AutoSkill_Delay", {
    Text    = "Delay Between Skills (s)",
    Default = 0.10,
    Min     = 0.02,
    Max     = 0.50,
    Rounding = 2,
...
TB_Tabs.MiscAuto.T2:AddSlider("AutoSkill_SelfHP", {
    Text    = "Min Self HP% to Use Skills",
    Default = 0,
    Min     = 0,
    Max     = 100,
    Rounding = 0,
    Tooltip = "0 = no check. Above 0: skip skills when your HP% is below this.",
})
...
TB_Tabs.Dungeon.T1:AddToggle("AutoInfinityTower", {
    ...
    GB.Player.Left.Server:AddSlider("AntiAFKInterval", {
        Text = "AFK Interval (giay)",
        Default = 55,
        Min = 20,
        Max = 90,
        Rounding = 0,
...
Toggles.AutoSeaFarm:OnChanged(function(state)
    if state then
        SetSeaFarmStatus("Starting...")
    else
        SetSeaFarmStatus("Disabled")
    end
    Thread("AutoSeaFarm", SafeLoop("Sea Farm", Func_SeaFarm), state)
end)
...
task.spawn(function()
    ...
    while true do
        local interval = (Options.AntiAFKInterval and Options.AntiAFKInterval.Value) or 55
        task.wait(interval)
...
-- ── Remote Config ──────────────────────────────────────────────────────────
local RemoteConfigGroup = Tabs.Config:AddRightGroupbox("Remote Config", "remoteconfig")

RemoteConfigGroup:AddLabel("Load config from URL (Pastebin raw, Github raw...)")

RemoteConfigGroup:AddInput("RemoteConfigURL", {
    Text = "Config URL",
    Default = "",
    Placeholder = "https://abcd.com/raw/xxxxxx",
    Numeric = false,
})

RemoteConfigGroup:AddButton("Load from URL", function()
    local url = Options.RemoteConfigURL and Options.RemoteConfigURL.Value or ""
    if not url or url == "" then
        Library:Notify("Input URL first!", 3)
        return
    end

    Library:Notify("Loading config from URL...", 3)
    ...
    if not ok or not result then
        Library:Notify("Cannot fetch URL! " .. tostring(result):sub(1, 80), 5)
        return
    end

    local ok2, data = pcall(HttpService.JSONDecode, HttpService, result)
    if not ok2 or type(data) ~= "table" then
        Library:Notify("JSON is not valid or wrong URL format!", 5)
        return
    end
    ...
    if type(data.Toggles) == "table" then
        for key, val in pairs(data.Toggles) do
            if Toggles[key] then
                local ok3 = pcall(function() Toggles[key]:SetValue(val) end)
    ...
    if type(data.Options) == "table" then
        for key, val in pairs(data.Options) do
            if Options[key] then
                local ok3 = pcall(function() Options[key]:SetValue(val) end)
                if ok3 then applied = applied + 1 end
            end
        end
    end

    Library:Notify("Applied " .. applied .. " setting from URL!", 5)
end)

RemoteConfigGroup:AddDivider()

RemoteConfigGroup:AddButton("Export Config (Copy JSON)", function()
    if not Support.Clipboard then
    ...
    for key, toggle in pairs(Toggles) do
        if toggle ~= nil and toggle.Value ~= nil then
            pcall(function() config.Toggles[key] = toggle.Value end)
        end
    end
    ...
    if ok and json then
        setclipboard(json)
        Library:Notify("Config copied! paste to Pastebin/Pastefy then copy raw", 6)
    else
        Library:Notify("Export failed!", 4)
    end
end)

RemoteConfigGroup:AddButton("Save Config to File", function()
    if not Support.FileIO then
        Library:Notify("Executor does not support FileIO!", 3)
        return
    end
    ...
    local ok, json = pcall(HttpService.JSONEncode, HttpService, config)
    if ok and json then
        local filename = "deluxe/configs/remote_export.json"
        pcall(function()
        ...
        if ok2 then
            Library:Notify("Saved: " .. filename, 5)
        else
            Library:Notify("File Overwrite failed!", 4)
        end
    else
        Library:Notify("Export failed!", 4)
    end
