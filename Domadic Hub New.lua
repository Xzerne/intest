...
local Windown = library.Create({
	Title = "Domadic Hub| [discord.gg/H59rXDs5]", Logo = 12523036534,
...
local CombatFramework = require(PBlade.PlayerScripts.CombatFramework)
local QBlade = getupvalues(CombatFramework)[2]
local RBlade = QBlade

function GetCurrentBlade()
    local controller = RBlade.activeController
    if not controller or not controller.blades then return end

    local blade = controller.blades[1]
    if not blade then return end

    while blade.Parent ~= PBlade.Character do
        blade = blade.Parent
        if not blade then return end
    end
...
function AttackNoCD()
    if _G.AutoFarmFruitMastery or _G.AutoFarmGunMastery then return end

    if _G.FastAttack then
        local controller = RBlade.activeController
        if not controller then return end

        local hits = require(game.ReplicatedStorage.CombatFramework.RigLib)
            .getBladeHits(PBlade.Character, {PBlade.Character.HumanoidRootPart}, 60)
        ...
        for _, v in pairs(hits) do
            ...
            if parent and parent:FindFirstChild("HumanoidRootPart") and not used[parent] then
                table.insert(targets, parent.HumanoidRootPart)
                used[parent] = true
            end
        end
        ...
        if #targets > 0 then
            pcall(function()
                if PBlade.Character:FindFirstChildOfClass("Tool")
                and controller.blades
                and controller.blades[1] then
