...
local function GetWorldBossTarget()
    ...
    if Toggles.BossesFarm.Value then
        ...
        for k, v in pairs(selected) do
            ...
            if enabled then
                for _, npc in pairs(PATH.Mobs:GetChildren()) do
                    if not table.find(Tables.MiniBossList, npc.Name)
                    and IsStrictBossMatch(npc.Name, bossName)
                    and IsValidTarget(npc) then
...
local Window = Library:CreateWindow({
	Title = "FRANKLIN",
	Footer = "" .. assetName .. " | DUMPED FROM CELINA",
	NotifySide = "Right",
...
TB_Tabs.Autofarm.T2:AddDropdown("ExceptBosses", {
...
if isfolder("taolao") then
    if not isfolder("celina") then
        makefolder("celina")
    end
    if not isfolder("celina/SailorPiece") then
        makefolder("celina/SailorPiece")
    end
    if not isfolder("celina/SailorPiece/settings") then
        makefolder("celina/SailorPiece/settings")
    end

    if isfolder("taolao/SailorPiece/settings") then
        for _, file in ipairs(listfiles("taolao/SailorPiece/settings")) do
            if isfile(file) then
                local filename = file:match("[^/\\]+$")
                local content = readfile(file)
                writefile("celina/SailorPiece/settings/" .. filename, content)
                delfile(file)
            end
        end
        delfolder("taolao/SailorPiece/settings")
    end
    
    if isfolder("taolao/SailorPiece") then
        for _, item in ipairs(listfiles("taolao/SailorPiece")) do
            if isfile(item) then
                local filename = item:match("[^/\\]+$")
                writefile("celina/SailorPiece/" .. filename, readfile(item))
                delfile(item)
            elseif isfolder(item) then
                local foldername = item:match("[^/\\]+$")
                if not isfolder("celina/SailorPiece/" .. foldername) then
                    makefolder("celina/SailorPiece/" .. foldername)
                end
                for _, subfile in ipairs(listfiles(item)) do
                    if isfile(subfile) then
                        local subfilename = subfile:match("[^/\\]+$")
                        writefile("celina/SailorPiece/" .. foldername .. "/" .. subfilename, readfile(subfile))
                        delfile(subfile)
                    end
                end
                delfolder(item)
        ...
        delfolder("taolao/SailorPiece")
    end
    
    for _, file in ipairs(listfiles("taolao")) do
        if isfile(file) then
            local filename = file:match("[^/\\]+$")
            writefile("celina/" .. filename, readfile(file))
            delfile(file)
        elseif isfolder(file) then
            local foldername = file:match("[^/\\]+$")
            if not isfolder("celina/" .. foldername) then
                makefolder("celina/" .. foldername)
            end
            for _, subfile in ipairs(listfiles(file)) do
                if isfile(subfile) then
                    local subfilename = subfile:match("[^/\\]+$")
                    writefile("celina/" .. foldername .. "/" .. subfilename, readfile(subfile))
                    delfile(subfile)
                end
            end
            delfolder(file)
